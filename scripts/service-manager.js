...
function getWindowsStartupPath() {
  ...
  return path.join(appData, 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'telegram-notifier.vbs');
}
...
function uninstallService() {
  ...
  if (platform === 'darwin') {
    ...
    const servicePath = getLinuxServicePath();
    try {
      execSync(`systemctl --user stop ${LINUX_SERVICE_NAME} 2>/dev/null || true`);
      execSync(`systemctl --user disable ${LINUX_SERVICE_NAME} 2>/dev/null || true`);
    } catch {
      // Ignore
    }
...
if (require.main === module) {
  ...
  if (cmd === 'install') {
    ...
    console.log('Usage: node scripts/service-manager.js [install|uninstall|status|restart]');
  }
}
