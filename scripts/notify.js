...
if (!BOT_TOKEN || !CHAT_ID) {
	console.error(
		"[Telegram Notifier] Error: TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID must be set in .env or environment variables.",
	);
	console.error(
		"Run `npm run setup` to configure your credentials interactively.",
	);
	process.exit(1);
}
...
if (
	...
	["error", "failed", "exception", "failure", "bug"].includes(rawType)
) {
	normalizedKey = "error";
}

const config = events[normalizedKey] ||
	events.info || { title: "Notification", emoji: "", label: "Notification" };
...
const shouldAddButtons =
	params.buttons ||
	(normalizedKey === "approval_required" && params["no-buttons"] !== true);
...
if (shouldAddButtons) {
	...
	inlineKeyboard = [
		[
			{
				text: buttons.approve || "Approve",
				callback_data: `approval:approve:${approvalId}`,
			},
			{
				text: buttons.reject || "Reject",
...
async function send() {
	try {
		...
		console.log(
			`[Telegram Notifier] Notification sent successfully (${eventLabel}).`,
		);
		process.exit(0);
	} catch (err) {
		console.error(
			`[Telegram Notifier] Failed to send notification: ${err.message}`,
		);
		process.exit(1);
	}
}
