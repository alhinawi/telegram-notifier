...
function isProjectDir(dirPath) {
  try {
    ...
    const indicators = ['.git', 'package.json', 'go.mod', 'Cargo.toml', 'pyproject.toml', 'requirements.txt', 'pom.xml', '.agents'];
...
function discoverProjects(maxCount = 10) {
  ...
  const sorted = Array.from(projectsMap.values()).sort((a, b) => b.mtime - a.mtime);
...
if (require.main === module) {
  ...
  console.log('\nDiscovered paths:');
  projects.forEach((p) => console.log(`- ${p.name}: ${p.path} (${p.branch || 'no git'})`));
}
