...
if (!botToken || botToken === "your_bot_token_here") {
	console.error(
		"[Telegram Notifier] TELEGRAM_BOT_TOKEN is missing or set to placeholder in .env",
	);
	...
	.get(`https://api.telegram.org/bot${botToken}/getUpdates`, (res) => {
		...
		res.on("end", () => {
			try {
				...
				if (!json.ok) {
					console.error(
						"[Telegram Notifier] Telegram API error:",
						json.description,
					);
					process.exit(1);
				}
				...
				if (updates.length === 0) {
					...
					console.log(
						"Please open Telegram, search for your bot, send /start or any message, then re-run this command:",
					);
