...
function printHeader() {
  console.log('\n========================================================');
  console.log('🚀 Telegram Notifier - Interactive Setup Wizard');
...
async function detectChatId(botToken) {
  ...
  for (let attempt = 1; attempt <= 10; attempt++) {
    ...
    process.stdout.write(`   [Attempt ${attempt}/10] Checking again in 3s...\r`);
...
function installGlobalPlugin(envConfig) {
  try {
    ...
    // Write global .env
    const globalEnvLines = [
      '# Global Telegram Notifier Configuration',
      `TELEGRAM_BOT_TOKEN=${envConfig.TELEGRAM_BOT_TOKEN}`,
      `TELEGRAM_CHAT_ID=${envConfig.TELEGRAM_CHAT_ID}`,
      `NOTIFICATION_LANGUAGE=${envConfig.NOTIFICATION_LANGUAGE}`,
    ];
    fs.writeFileSync(path.join(globalPluginDir, '.env'), globalEnvLines.join('\n'), 'utf8');
...
function installLocalProject(targetDir, envConfig) {
  try {
    const skillsDir = path.join(targetDir, '.agents', 'skills', 'telegram-notifier');
    fs.mkdirSync(skillsDir, { recursive: true });

    const skillContent = fs.readFileSync(path.join(ROOT_DIR, 'skills', 'telegram-notifier', 'SKILL.md'), 'utf8');
    ...
    const ruleSnippet = [
      ...
      `node "${path.join(ROOT_DIR, 'scripts', 'notify.js').replace(/\\/g, '/')}" --type=task_finished --title="Finished"`,
      '',
    ].join('\n');
    ...
    console.log(`📁 Local project integration added to: ${targetDir}`);
    return true;
  } catch (err) {
...
async function main() {
  ...
  console.log('  [4] Standalone (Only keep repository files)');
  ...
  console.log('\n========================================================');
  console.log('✅ Setup Complete! Your AI Agents can now notify your phone.');
  console.log('========================================================\n');
