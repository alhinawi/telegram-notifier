...
async function detectChatId(botToken) {
	...
	console.log(
		"Please open Telegram, search for your bot, and send /start or any message.",
	);
	...
	for (let attempt = 1; attempt <= 15; attempt++) {
		...
		process.stdout.write(
			`   [Attempt ${attempt}/15] Checking again in 3s...\r`,
		);
...
async function main() {
	...
	if (
		...
		(!existingEnv.TELEGRAM_BOT_TOKEN ||
			existingEnv.TELEGRAM_BOT_TOKEN === "your_bot_token_here")
	) {
		console.log(
			"Non-interactive environment detected. Run in an interactive terminal to configure.",
		);
		rl.close();
		return;
	}
	...
	if (botToken && botToken !== "your_bot_token_here") {
		const useExisting = await ask(
			`Found existing Bot Token (${botToken.slice(0, 8)}...). Use it? [Y/n]: `,
		);
	...
	while (!botToken || botToken === "your_bot_token_here") {
		console.log(
			"Enter your Telegram Bot Token from @BotFather (e.g. 1234567890:ABCdefGh...):",
		);
	...
	const langChoice = await ask(
		`Choose language [1-${availableLangs.length}] (Default: 1): `,
	);

	const choiceIdx = parseInt(langChoice.trim(), 10) - 1;
	const lang =
		(availableLangs[choiceIdx] && availableLangs[choiceIdx].code) || "en";
	...
	if (lang === "ar") {
		console.log(
			"ملاحظة: كتابة المسارات تبدأ من مجلد المستخدم (~) أو المسار الكامل، مثل: ~/workspace",
		);
	} else {
		console.log(
			"Note: Paths should start from your Home directory (~) or full absolute path, e.g. ~/workspace",
		);
	}
	...
	try {
		...
		console.log(
			"Test notification delivered successfully! Check your Telegram.",
		);
	} catch (err) {
	...
	console.log(
		"Installing and auto-starting OS background service automatically...",
	);
	serviceManager.installService();

	console.log("\n========================================================");
	console.log(
		"Setup Complete! Your AI Agents will now notify your phone automatically.",
	);
	console.log("========================================================\n");
